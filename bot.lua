...
local res = http.request(database.."joke.db")
 ...
 tdcli.sendText(chat_id, msg.id_, 0, 1, nil, run..'\n\n*TeleGram Member: @tgMember', 1, 'md')
      ...
      if input:match("^[#!/][Ss]ettings$") and is_mod(msg) and groups or input:match("^[Ss]ettings$") and is_mod(msg) and groups or input:match("^تنظیمات$") and is_mod(msg) and groups then
        ...
        .."🏅 #ممنوعیت متن : ".."`"..text1.."`".."\n"
        .."🏅 ورژن 6.0 تلگرام گارد - @tgMember"
        tdcli.sendText(chat_id, msg.id_, 0, 1, nil, text, 1, 'md')
      end
			    if input:match("^[#!/][Tt]g[Gg]uard$") and is_mod(msg) and groups or input:match("^[#!/][Aa]bout$") and is_mod(msg) and groups or input:match("^[Aa]bout$") and is_mod(msg) and groups then
        local text = "👥 `تلگرام گارد ورژن 6.0`:".."\n"
				.."🏅 `تلگرام گارد رباتي قدرتمند جهت مديريت سوپرگروه`\n"
...
if input:match("^[#!/][Hh]elp$") and is_mod(msg) or input:match("^[Hh]elp$") and is_mod(msg) or input:match("^راهنما$") and is_mod(msg) then
        local text = "🏅`راهنماي فارسي انگليسي ورژن 6.0`:".."\n"
	.."🏅قفل ها:\n"
...
if input:match("^[#!/][Tt][Gg][Gg][Uu][Aa][Rr][Dd]$") and is_mod(msg) or input:match("^[Tt]g[Gg]uard$") and is_mod(msg) or input:match("^تلگرام گارد$") and is_mod(msg) then
        local text = "👥 `تلگرام گارد ورژن 6.0`:".."\n"
				.."🏅 `تلگرام گارد رباتي قدرتمند جهت مديريت سوپرگروه`\n"
        .."🏅 `نوشته شده برپايه tdcli(New TG)` \n"
        .."🏅 `بيس = tgGuard` \n"
...
if input:match("^ربات$") and is_mod(msg) or input:match("^[Tt]g[Gg]uard$") and is_mod(msg) or input:match("^آنلاینی$") and is_mod(msg) then
        local text = [[_tgGuard Version: 6.0
